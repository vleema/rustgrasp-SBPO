...
impl Symbionts {
    ...
    fn evolve(
        ...
        let subpop = &mut self.pop[r1..r2];
        for (p, locks) in subpop.iter_mut().zip(self.blocks.iter_mut()) {
            if is_panic {
                ...
                let candidates: Vec<&DynTransgeneticVector> = tv
                    .iter()
                    .filter(|agent| {
                        let is_plasmid = matches!(
                            agent,
                            DynTransgeneticVector::Plasmid(_) | DynTransgeneticVector::Virus(_)
                        );
